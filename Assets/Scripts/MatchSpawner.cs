...
public class MatchSpawner : MonoBehaviour
{
    ...
    public void fillColorMap()
    {
        ...
        for (int i = 0; i < SIZE_OF_TEXTURE; ++i)
        {
            ...
            {
                if (i % (SIZE_OF_TEXTURE / 10) == 0) colorMap[j + i * SIZE_OF_TEXTURE] = new Color(53 / 255f, 75 / 255f, 98 / 255f);
    ...
    void Update()
    {
        ...
        if (recalculationCyclesCounter % (10 * FREQUENCY) == 0 && localMatches+leftoverMatches < MAX_MATCHES)
        {
            ...
            // if (localMatches+leftoverMatches > 0 && localGoodMatches+leftoverGoodMatches > 0)
    ...
    public int checkMatchesPosition()
    {
        ...
        for (int i = 0; i < localMatches; ++i)
        {
            ...
            for (int j = -PLANE_TICKS / 2; j <= PLANE_TICKS / 2; ++j)
            {
                ...
                {
                    if (Mathf.Abs(matches[i].transform.position.z - j) <= 
            ...
            if (((tmpX >= 0 && tmpX <= 45) || (tmpX >= 315 && tmpX <= 360)) || (tmpX >= 135 && tmpX <= 225))
            {
                ...
                Mathf.Abs(Mathf.Sin(matches[i].transform.eulerAngles.y * Mathf.PI / 180)) * 0.5f / 2) 
                {
                    ...
                    (
                        ...
                        tmpX + ". Case 0, 180, 360:" + 
                        Mathf.Abs(matches[i].transform.position.z - j) + " <= " + Mathf.Abs(Mathf.Sin(matches[i].transform.eulerAngles.y * Mathf.PI / 180)) * 0.5f / 2
                    );
            ...
            else if (tmpX > 45 && tmpX < 135)
            {
                ...
                Mathf.Abs(Mathf.Cos(Mathf.Abs(matches[i].transform.eulerAngles.y - matches[i].transform.eulerAngles.z) * Mathf.PI / 180)) * 0.5f / 2) 
                {
                    ...
                    (
                        ...
                        "     posZ: " + matches[i].transform.position.z + '\n' +
                        tmpX + ". Case 90:" + 
                        Mathf.Abs(matches[i].transform.position.z - j) + "<= " +
                        Mathf.Abs(Mathf.Cos(Mathf.Abs(matches[i].transform.eulerAngles.y - matches[i].transform.eulerAngles.z) * Mathf.PI / 180)) * 0.5f / 2
                    );
...
/*
float tmpX = Mathf.Abs(matches[i].transform.localEulerAngles.x);
                // near 0 degrees or near 180 degrees
                if (((tmpX >= 0 && tmpX <= 45) || (tmpX >= 315 && tmpX <= 360)) || (tmpX >= 135 && tmpX <= 225))
                {
                    if (Mathf.Abs(matches[i].transform.position.z - j) <= 
                    Mathf.Abs(Mathf.Sin(Mathf.Abs(matches[i].transform.localEulerAngles.y - matches[i].transform.localEulerAngles.z) * Mathf.PI / 180)) * 0.5f / 2) 
                    {
                        // if (i == localMatches - 1) Debug.Log( 
                        // "111  anX: " + matches[i].transform.localEulerAngles.x +
                        // "     anY: " + matches[i].transform.localEulerAngles.y +
                        // "     anZ: " + matches[i].transform.localEulerAngles.z
                        // );
                        succesessMatches++;
                        if (matches[i].transform.position.y < 3) matches[i].GetComponent<Renderer>().material.color = new Color(0f/255, 60f/255, 230f/255);
                        break;
                ...
                {
                    if (Mathf.Abs(matches[i].transform.position.z - j) <= 
                    Mathf.Abs(Mathf.Cos(Mathf.Abs(matches[i].transform.localEulerAngles.y - matches[i].transform.localEulerAngles.z) * Mathf.PI / 180)) * 0.5f / 2) 
                    {
                        // if (i == localMatches - 1) Debug.Log( 
                        // "222  anX: " + matches[i].transform.localEulerAngles.x +
                        // "     anY: " + matches[i].transform.localEulerAngles.y +
                        // "     anZ: " + matches[i].transform.localEulerAngles.z
                        // );
                        succesessMatches++;
                ...
                else if (tmpX > 225 && tmpX < 315)
                {
                    ...
                    {
                        // if (i == localMatches - 1) Debug.Log( 
                        // "333  anX: " + matches[i].transform.localEulerAngles.x +
                        // "     anY: " + matches[i].transform.localEulerAngles.y +
                        // "     anZ: " + matches[i].transform.localEulerAngles.z
                        // );
                        succesessMatches++;
...
/*
                ...
                if (((tmpX >= 0 && tmpX <= 45) || (tmpX >= 315 && tmpX <= 360)) || (tmpX >= 135 && tmpX <= 225))
                {
                    ...
                    if (Mathf.Abs(matches[i].transform.position.z - j*(1.0f * 2 * SIZE_Z / PLANE_TICKS)) <= 
                        Mathf.Abs(Mathf.Sin(tmp / 180 * 2 * Mathf.PI)) * 0.5f / 2) 
                    {
                        succesessMatches++;
                        if (matches[i].transform.position.y < 3) matches[i].GetComponent<Renderer>().material.color = new Color(0f/255, 60f/255, 230f/255);
                        break;
                ...
                else
                {
                    ...
                    if (Mathf.Abs(matches[i].transform.position.z - j*(1.0f * 2 * SIZE_Z / PLANE_TICKS)) <= 
                    Mathf.Abs(Mathf.Cos(tmp / 180 * 2 * Mathf.PI)) * 0.5f / 2) 
                    {
                        succesessMatches++;
                        if (matches[i].transform.position.y < 3) matches[i].GetComponent<Renderer>().material.color = new Color(0f/255, 60f/255, 230f/255);
                        break;
...
float deltaX = findDeltaToMiddles(matches[i].transform.localEulerAngles.x);
                ...
                if ((xWasBest && 
                        (
                        angleY >= 269 && angleY <= 271 && 
                        angleZ >= 179 && angleZ <= 181
                        ) 
                        || 
                        (
                        angleZ >= 269 && angleZ <= 271 && 
                        angleY >= 179 && angleY <= 181
                        )
                    ) ||
                    (yWasBest && 
                        (
                        angleX >= 269 && angleX <= 271 && 
                        angleZ >= 179 && angleZ <= 181
                        ) 
                        || 
                        (
                        angleZ >= 269 && angleZ <= 271 && 
                        angleX >= 179 && angleX <= 181
                        )
                    ) ||
                    (zWasBest && 
                        (
